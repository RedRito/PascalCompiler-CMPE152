...
{
    ParserNode *currTerm = nullptr;
    
    if (readToken->datatype == PToken::PLUSOP)
    {
        readToken = scanner->nextToken();  // consume +
        currTerm = parseFactor();
    }
    else if (readToken->datatype == PToken::MINUSOP)
    {
        readToken = scanner->nextToken();  // consume -
        currTerm = new ParserNode(NodeType::NEGATE);
        currTerm->adopt(parseFactor());
    }
    else currTerm = parseFactor();
    ...
    while(termOperators.find(readToken->datatype) != termOperators.end())
    {
        ...
        // if(ParserNode *opNode = readToken->datatype == STAR)
        // {
        ...
        if (readToken->datatype == PToken::COLON)
        {
            ...
            if (readToken->datatype == PToken::INTEGER)
            {
                ...
                if (readToken->datatype == PToken::COLON)
                {
                    ...
                    else printSyntax("Invalid count of decimal places");
                }
            }
            else printSyntax("Invalid field width");
        }
    }
